...
Given /^these articles exist in the article collection "([^"]*)"$/ do |collection_name, table|
  ...
  table.hashes.each do |hash|
    @article_collection.articles << Factory.build(:article, :title => hash[:title], :created_by_id => @article_collection.created_by_id, :updated_by_id => @article_collection.updated_by_id)
