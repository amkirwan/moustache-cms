...
describe "admin/theme_assets/_form.html.haml" do
  ...
  context "when the theme_asset is NEW" do
    ...
    it "should render a field to upload file" do
      form_new(:action => admin_theme_assets_path) do |f|
        f.should have_selector("input", :type => "file", :name => "theme_asset[source]")
      end
    end
    
    it "should render a hidden field to cache the file tmp path on redisplay" do
      form_new(:action => admin_theme_assets_path) do |f|
        f.should have_selector("input", :type => "hidden", :name => "theme_asset[source_cache]")
      end
    end
  end
  ...
  context "when redisplaying new field form because of validation error should cache the image source from tmp" do
    ...
    it "should show the cached image" do
      theme_asset.stub(:source? => true)
      theme_asset.stub_chain(:source, :thumb, :url => "/spec/fixtures/assets/rails.png")
      do_render("Save Theme Asset")
      rendered.should have_selector("img", :src => "/spec/fixtures/assets/rails.png")
    end
    
    it "should cache the file tmp path on redisplay" do
      theme_asset.stub(:source_cache => "/tmp/rails.png")
      do_render("Save Theme Asset")
      form_new(:action => admin_theme_assets_path) do |f|
        f.should have_selector("input", :type => "hidden", :name => "theme_asset[source_cache]", :value => "/tmp/rails.png")
      end
    end
  end
  ...
  context "when EDITing the theme_asset" do
    ...
    it "should show the location of the file" do
      theme_asset.stub_chain(:source, :url => "/image/path/file.png")
      do_render("Update Asset")
      rendered.should have_selector("a", :content => "http://#{site.full_subdomain}#{theme_asset.source.url}")
    end 
